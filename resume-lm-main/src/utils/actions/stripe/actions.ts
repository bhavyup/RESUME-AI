...
import { isStripeEnabled } from '@/lib/feature-flags';

// Initialize stripe only if enabled
const stripe = isStripeEnabled() ? new Stripe(process.env.STRIPE_SECRET_KEY!, {
...
function ensureStripeEnabled() {
  if (!isStripeEnabled() || !stripe) {
    throw new Error('Stripe is not enabled. Please set STRIPE_SECRET_KEY in your environment variables.');
  }
  return stripe;
}
...
async function createCustomerInStripe(uuid: string, email: string): Promise<string> {
  const stripeClient = ensureStripeEnabled();
...
export async function manageSubscriptionStatusChange(
  ...
  const stripeClient = ensureStripeEnabled();
  const customerData = await stripeClient.customers.retrieve(customerId);
...
export async function deleteCustomerAndData(uuid: string) {
  ...
  if (subscription?.stripe_customer_id) {
    ...
    const stripeClient = ensureStripeEnabled();
    await stripeClient.customers.del(subscription.stripe_customer_id);
  }
