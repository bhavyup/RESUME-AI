...
export async function generate(input: string, config?: AIConfig) {
  try {
   ...
   `;
         ...
         // Stream and wait for the model to finish, accumulate plain string locally
         let finalText = '';
         for await (const delta of textStream) {
            // delta may be string chunks
            finalText += delta;
            stream.update(delta);
         }
         ...
         // Return the accumulated plain string so the client receives a serializable value
