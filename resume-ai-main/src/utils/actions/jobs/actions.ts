...
export async function createJob(jobListing: z.infer<typeof simplifiedJobSchema>) {
  ...
  const jobData = {
    // Ensure DB `title` (not-null) is provided; map from legacy `position_title` if present
    title: jobListing.position_title ?? jobListing.title ?? 'Untitled',
    user_id: user.id,
...
export async function createEmptyJob(): Promise<Job> {
  ...
  const emptyJob: Partial<Job> = {
    ...
    // ensure `title` not-null constraint satisfied
    title: 'New Position',
    job_url: null,
    description: null,
    location: null,
