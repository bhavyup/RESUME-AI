...
export async function tailorResumeToJob(
  ...
  // Determine model to use: prefer provided config, otherwise server default
  const modelToUse = config?.model && config.model.trim() ? config.model : getDefaultModel(isPro);
  const effectiveConfig: AIConfig = {
    model: modelToUse,
    apiKeys: config?.apiKeys || [],
  };
  const aiClient = initializeAIClient(effectiveConfig, isPro, true);
// Check rate limit
