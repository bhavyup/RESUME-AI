...
export function SplitContent({
  ...
  return (
    ...
    <div className={cn(
      ...
      "before:absolute before:inset-0 before:bg-gradient-to-br before:from-violet-50/80 before:via-blue-50/80 before:to-indigo-50/80 before:opacity-0",
      "after:absolute after:inset-0 after:bg-[linear-gradient(to_right,#8882_1px,transparent_1px),linear-gradient(to_bottom,#8882_1px,transparent_1px)] after:bg-[size:14px_24px] after:opacity-0 ",
      className
    )}>
                ...
                <div className="space-y-2 flex flex-col items-end">
                  {bulletPoints.map((point, idx) => (
                    <div key={idx} className="flex items-start gap-2 flex-row-reverse">
                      ...
                      <span className="text-sm md:text-base text-slate-700 text-right">{point}</span>
                    </div>
                  ))}
                </div>
              )}
