  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      createBaseResume(
        ...
        // Navigate immediately - Suspense will show loading skeleton
        router.push(`/resumes/${resume.id}`);
      }).catch(() => {
        toast({
          title: "Error",
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent
        ...
        className={cn(
          ...
          "bg-slate-900 border border-slate-700 shadow-2xl shadow-emerald-500/5 rounded-xl"
        )}>
        <style jsx global>{`
          @keyframes shake {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className={cn(
                ...
                currentStep >= 1 ? "bg-emerald-500 text-white" : "bg-slate-700 text-slate-400"
              )}>
                1
              </div>
              <div className={cn(
              ...
              <div className={cn(
                "w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium",
                currentStep >= 2 ? "bg-emerald-500 text-white" : "bg-slate-700 text-slate-400"
              )}>
                2
              </div>
            </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="text-center space-y-2">
                <h3 className="text-xl font-semibold text-white">What role are you targeting?</h3>
                <p className="text-slate-400">This helps us tailor your resume content and format</p>
              </div>
              
              <div className="space-y-4 max-w-md mx-auto">
                <div className="space-y-2">
                  <Label htmlFor="target-role" className="text-sm font-medium text-slate-200">
                  ...
                  <Input
                    ...
                    className={cn(
                      "h-12 text-base bg-slate-800/50 border-slate-600 text-white placeholder:text-slate-500",
                      "focus:border-emerald-500 focus:ring-emerald-500/20",
                      isTargetRoleInvalid && "border-rose-500 shake"
                    )}
                    required
                    autoFocus
                  />
                ...
                <div className="text-xs text-slate-500 space-y-1">
                  <p className="text-slate-400">💡 <strong>Tips for better results:</strong></p>
          ...
          {currentStep === 2 && (
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <Label className="text-sm font-medium text-slate-200">Resume Content</Label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={option.id}>
                      ...
                      >
                        <option.icon className="w-5 h-5 text-emerald-400 mb-2" />
                        <div className="text-xs font-medium text-center">
                          <div className="text-slate-200">{option.label}</div>
                          <div className="text-slate-500 mt-0.5">{option.desc}</div>
                        </div>
                      </Label>
                    </div>
                  ))}
                ...
                {importOption === 'import-profile' && (
                  <div className="mt-4 space-y-3">
                    <div className="text-sm font-medium text-slate-200">Select Content to Include</div>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <Accordion key={section.key} type="single" collapsible>
                          <AccordionItem value={section.key} className="border border-slate-700 rounded-lg bg-slate-800/30">
                            <div className="flex items-center gap-2 px-3 py-2">
                              <Checkbox
                                ...
                                className={cn(
                                  "border-slate-600 data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500",
                                  isSectionPartiallySelected(section.key as keyof typeof selectedItems) && "data-[state=checked]:bg-emerald-500/50"
                                )}
                              />
                              <AccordionTrigger className="flex-1 py-0 hover:no-underline text-slate-200">
                                <div className="flex items-center justify-between w-full">
                                  <span className="text-sm font-medium">{section.label}</span>
                                  <span className="w-2"></span>
                                  <span className="text-xs text-slate-500">{section.data.length}</span>
                                </div>
                              </AccordionTrigger>
                            </div>
                            <AccordionContent className="px-3 pb-3">
                              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                                {section.data.map((item: WorkExperience | Education | Skill | Project, index: number) => {
                                  ...
                                  return (
                                    <div key={id} className="flex items-center gap-2 p-2 rounded bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
                                      <Checkbox
                                      ...
                                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => handleItemSelection(section.key as keyof typeof selectedItems, id)}>
                                        {section.key === 'work_experience' && (
                                          <div>
                                            <div className="text-xs font-medium truncate text-slate-200">{(item as WorkExperience).position}</div>
                                            <div className="text-xs text-slate-500">{(item as WorkExperience).company} • {(item as WorkExperience).date}</div>
                                          </div>
                                        )}
                                        {section.key === 'projects' && (
                                          <div>
                                            <div className="text-xs font-medium truncate text-slate-200">{(item as Project).name}</div>
                                            {(item as Project).technologies?.length && (
                                              <div className="text-xs text-slate-500 truncate">{(item as Project).technologies?.slice(0, 2).join(', ')}</div>
                                            )}
                                          </div>
                                        )}
                                        {section.key === 'education' && (
                                          <div>
                                            <div className="text-xs font-medium truncate text-slate-200">{(item as Education).degree} in {(item as Education).field}</div>
                                            <div className="text-xs text-slate-500">{(item as Education).school} • {(item as Education).date}</div>
                                          </div>
                                        )}
                                        {section.key === 'skills' && (
                                          <div>
                                            <div className="text-xs font-medium text-slate-200">{(item as Skill).category}</div>
                                            <div className="flex flex-wrap gap-1 mt-1">
                                              {(item as Skill).items.slice(0, 3).map((skill: string, index: number) => (
                                                <Badge key={index} variant="secondary" className="text-[10px] px-1 py-0 bg-slate-700 text-slate-300 border-slate-600">
                                                  {skill}
                                                </Badge>
                                              ))}
                                              {(item as Skill).items.length > 3 && (
                                                <span className="text-[10px] text-slate-500">+{(item as Skill).items.length - 3} more</span>
                                              )}
                                            </div>
                                          </div>
                                        )}
                  ...
                  <div className="space-y-3">
                    <label
                      ...
                      className={cn(
                        ...
                        isDragging ? "border-emerald-500 bg-emerald-500/10" : "border-slate-600 hover:border-emerald-500/50"
                      )}
                    >
                      <input type="file" className="hidden" accept="application/pdf" onChange={handleFileInput} />
                      <Upload className="w-8 h-8 text-emerald-400" />
                      <div className="text-center">
                        <p className="text-sm font-medium text-slate-200">Drop PDF here or click to browse</p>
                    ...
                    <Textarea
                      ...
                      className="min-h-[120px] text-sm bg-slate-800/50 border-slate-600 text-white placeholder:text-slate-500"
                    />
                  </div>
                )}
              </div>
        ...
        <div className="px-6 py-4 border-t border-slate-700/50 bg-slate-800/30">
          <div className="flex justify-between">
            ...
            <div className="flex gap-2">
              ...
              {currentStep === 1 && (
                <Button onClick={handleNext} size="sm" className="bg-emerald-500 hover:bg-emerald-600 text-white">
                  Next
                </Button>
              )}
              {currentStep === 2 && (
                <Button onClick={handleCreate} disabled={isCreating} size="sm" className="bg-emerald-500 hover:bg-emerald-600 text-white">
                  {isCreating ? (
                    <>
