"use client";

import { useState } from "react";
import {
  Dialog,
  DialogContent,
  DialogTitle,
  DialogTrigger,
  DialogDescription,
} from "@/components/ui/dialog";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { LoginForm } from "@/components/auth/login-form";
import { SignupForm } from "@/components/auth/signup-form";
import { Button } from "@/components/ui/button";
import { ArrowRight, Github, Loader2 } from "lucide-react";
import { AuthProvider } from "./form-validation-context";
import { Separator } from "@/components/ui/separator";
import { authService } from "@/lib/auth";

const gradientClasses = {
  base: "bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-500",
  hover: "hover:from-amber-400 hover:via-yellow-400 hover:to-amber-400",
  shadow: "shadow-lg shadow-amber-500/30",
  animation: "transition-all duration-500 animate-gradient-x",
};

interface TabButtonProps {
  value: "login" | "signup";
  children: React.ReactNode;
}

interface AuthDialogProps {
  children?: React.ReactNode;
}

function TabButton({ value, children }: TabButtonProps) {
  return (
    <TabsTrigger
      value={value}
      className="
        relative flex-1 h-9 px-3 text-sm font-medium rounded-lg
        transition-all duration-200 ease-out
        data-[state=inactive]:text-slate-400 data-[state=inactive]:bg-transparent
        data-[state=active]:text-amber-400 data-[state=active]:bg-slate-800/50 data-[state=active]:shadow-md
        data-[state=inactive]:hover:text-amber-300 data-[state=inactive]:hover:bg-slate-800/30
        border-0 shadow-none
        focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:ring-offset-0
      "
    >
      {children}
    </TabsTrigger>
  );
}

function SocialAuth() {
  const [githubLoading, setGithubLoading] = useState(false);
  const [googleLoading, setGoogleLoading] = useState(false);

  const handleGithubSignIn = () => {
    setGithubLoading(true);
    // Redirect to Spring Boot OAuth endpoint
    window.location.href = authService.getOAuthUrl("github");
  };

  const handleGoogleSignIn = () => {
    setGoogleLoading(true);
    // Redirect to Spring Boot OAuth endpoint
    window.location.href = authService.getOAuthUrl("google");
  };

  return (
    <div className="space-y-3 mt-4">
      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <Separator className="bg-slate-700" />
        </div>
        <div className="relative flex justify-center text-xs">
          <span className="bg-slate-900 px-3 text-slate-500">
            or continue with
          </span>
        </div>
      </div>

      {/* Google Sign In */}
      <Button
        variant="outline"
        type="button"
        className="
          w-full h-11 bg-slate-800/50 border-slate-700 hover:bg-slate-800 hover:border-amber-500/30
          text-slate-200 font-medium transition-all duration-200
          focus:ring-2 focus:ring-amber-500/50 focus:ring-offset-0
          rounded-lg group backdrop-blur-sm
          hover:shadow-lg hover:shadow-amber-500/10
        "
        onClick={handleGoogleSignIn}
        disabled={googleLoading}
      >
        {googleLoading ? (
          <>
            <Loader2 className="mr-2 h-5 w-5 animate-spin text-amber-400" />
            <span className="text-slate-300">Connecting...</span>
          </>
        ) : (
          <>
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            <span className="group-hover:text-amber-300 transition-colors">
              Continue with Google
            </span>
          </>
        )}
      </Button>

      {/* GitHub Sign In */}
      <Button
        variant="outline"
        type="button"
        className="
          w-full h-11 bg-slate-800/50 border-slate-700 hover:bg-slate-800 hover:border-emerald-500/30
          text-slate-200 font-medium transition-all duration-200
          focus:ring-2 focus:ring-emerald-500/50 focus:ring-offset-0
          rounded-lg group backdrop-blur-sm
          hover:shadow-lg hover:shadow-emerald-500/10
        "
        onClick={handleGithubSignIn}
        disabled={githubLoading}
      >
        {githubLoading ? (
          <>
            <Loader2 className="mr-2 h-5 w-5 animate-spin text-emerald-400" />
            <span className="text-slate-300">Connecting...</span>
          </>
        ) : (
          <>
            <Github className="mr-2 h-5 w-5 text-slate-300 group-hover:text-emerald-400 transition-colors" />
            <span className="group-hover:text-emerald-300 transition-colors">
              Continue with GitHub
            </span>
          </>
        )}
      </Button>
    </div>
  );
}

export function AuthDialog({ children }: AuthDialogProps) {
  const [open, setOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<"login" | "signup">("login");

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {/* AUTH DIALOG TRIGGER BUTTON */}
      <DialogTrigger asChild>
        {children || (
          <Button
            size="lg"
            className={`${gradientClasses.base} ${gradientClasses.hover} text-slate-900 font-bold 
            text-lg py-6 px-10 ${gradientClasses.animation} group
            shadow-xl ${gradientClasses.shadow} hover:shadow-amber-500/50
            ring-2 ring-amber-500/20 hover:ring-amber-500/40
            scale-105 hover:scale-110 transition-all duration-300
            rounded-xl relative overflow-hidden`}
            aria-label="Open authentication dialog"
          >
            <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <span className="relative z-10 flex items-center justify-center">
              Start Now
              <ArrowRight className="ml-3 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </span>
          </Button>
        )}
      </DialogTrigger>

      <DialogContent
        className="
          sm:max-w-[440px] w-full max-h-[90vh] p-0 
          bg-slate-900/95 backdrop-blur-xl border border-slate-800/50 shadow-2xl shadow-black/40
          animate-in fade-in-0 zoom-in-95 duration-200
          rounded-2xl overflow-hidden overflow-y-auto
        "
      >
        <AuthProvider>
          {/* Hidden accessibility elements */}
          <DialogTitle className="sr-only">Authentication</DialogTitle>
          <DialogDescription className="sr-only">
            Sign in or create an account
          </DialogDescription>

          {/* Decorative gradient line at top */}
          <div className="h-1 w-full bg-gradient-to-r from-amber-500 via-yellow-500 to-emerald-500"></div>

          {/* Content starts with tabs */}
          <div className="px-6 pt-6 pb-8">
            <Tabs
              value={activeTab}
              onValueChange={(value) =>
                setActiveTab(value as "login" | "signup")
              }
              className="w-full"
            >
              <TabsList
                className="
                w-full h-12 bg-slate-800/30 border border-slate-700/50 p-1.5
                flex gap-1 rounded-xl backdrop-blur-sm
              "
              >
                <TabButton value="login">Sign In</TabButton>
                <TabButton value="signup">Create Account</TabButton>
              </TabsList>

              {/* Forms Content */}
              <div className="mt-6 pb-2">
                <TabsContent value="login" className="mt-0 space-y-5">
                  <div className="text-center mb-5">
                    <h3 className="text-xl font-bold bg-gradient-to-r from-amber-400 to-yellow-500 bg-clip-text text-transparent">
                      Welcome back
                    </h3>
                    <p className="text-sm text-slate-400 mt-1.5">
                      Sign in to continue building your resume
                    </p>
                  </div>
                  <LoginForm />
                  <SocialAuth />
                </TabsContent>

                <TabsContent value="signup" className="mt-0 space-y-5">
                  <div className="text-center mb-5">
                    <h3 className="text-xl font-bold bg-gradient-to-r from-emerald-400 to-teal-500 bg-clip-text text-transparent">
                      Get started
                    </h3>
                    <p className="text-sm text-slate-400 mt-1.5">
                      Create your free account in seconds
                    </p>
                  </div>
                  <SignupForm />
                  <SocialAuth />
                </TabsContent>
              </div>
            </Tabs>

            {/* Privacy note */}
            <p className="text-xs text-center text-slate-500 mt-6 px-4">
              By continuing, you agree to our Terms of Service and Privacy
              Policy
            </p>
          </div>
        </AuthProvider>
      </DialogContent>
    </Dialog>
  );
}
